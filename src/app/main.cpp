...
// Soft-start variables (Commented out temporarily)
// bool first_run_after_init = true;  // To only apply soft-start on first run
bool first_run_after_init = false;  // To only apply soft-start on first run (set to false to disable soft start)
...
float currentTargetSpeed = 0.0;          // Current target speed that we're accelerating toward
float currentActualSpeed = 0.0;          // Current actual speed with acceleration applied
uint32_t lastAccelerationTime = 0;       // Timestamp for acceleration calculations
...
// Function to calculate speed with acceleration/deceleration limits
float calculateAcceleratedSpeed(float targetSpeed) {
  uint32_t currentTime = millis();
  float deltaTime = (currentTime - lastAccelerationTime) / 1000.0; // Convert to seconds
  
  // Initialize on first call
  if (lastAccelerationTime == 0) {
    currentActualSpeed = 0.0;  // Start from 0
    lastAccelerationTime = currentTime;
    return currentActualSpeed;
  }
  
  if (deltaTime > 0) {
    float speedDifference = targetSpeed - currentActualSpeed;
    
    if (speedDifference > 0) {
      // Accelerating - increasing speed
      float maxSpeedIncrease = MOTOR_ACCELERATION * deltaTime;
      if (speedDifference <= maxSpeedIncrease) {
        currentActualSpeed = targetSpeed;  // Reach target
      } else {
        currentActualSpeed += maxSpeedIncrease;  // Continue accelerating
      }
    } else if (speedDifference < 0) {
      // Decelerating - decreasing speed
      float maxSpeedDecrease = MOTOR_DECELERATION * deltaTime;
      if (-speedDifference <= maxSpeedDecrease) {
        currentActualSpeed = targetSpeed;  // Reach target
      } else {
        currentActualSpeed -= maxSpeedDecrease;  // Continue decelerating
      }
    }
...
void loop() {
  ...
  // Update the motor driver with the current speed for PID control
  ...
  Serial.printf(
    ...
    sonarDistance, servoAngle, motorPWM, currentSpeed, lineFollowError
  );
  #endif
  ...
  switch (state) {
    ...
    case INIT:
      setMotorSpeed(0.0);  // Stop the motor (error checking not required for normal operation)
      ...
      if (digitalRead(BTN_BOOT) == LOW) {
        ...
        // first_run_after_init = true;  // Enable soft start for the first run (Commented out)
        setMotorSpeed(1.0);
    ...
    case LINE_FOLLOW: {
        ...
        // Commenting out soft start functionality temporarily
        /*
        if (first_run_after_init) {
            ...
            float softStartSpeedValue;
            if (getSoftStartSpeed(PWM_NORMAL, &softStartSpeedValue) != ERROR_SUCCESS) {
                softStartSpeedValue = PWM_NORMAL; // Fallback to normal if failed
            }
            setMotorSpeed(softStartSpeedValue);
            motorPWM = static_cast<int16_t>(softStartSpeedValue / SPEED_PER_PWM); // Update motorPWM for comparison
        }
        */
        // Use direct motor speed control instead of soft start
        setMotorSpeed(1.0);
        motorPWM = PWM_NORMAL; // Update motorPWM to normal value

        // Update motor only if PWM changed
        if (motorPWM != lastMotorPWM) {
            setMotorSpeed(motorPWM * SPEED_PER_PWM);  // Convert PWM to speed in m/s
    ...
    case AVOID_PREPARE:
      if (avoid_prepare_start == 0) {
        setMotorSpeed(0.3); // Attempt to set motor speed (0.3 m/s)
    ...
    case AVOID_PATH:
      {
          // Calculate distance traveled based on encoder counts
          float wheelRev = encoderCount / COUNTS_PER_REV;  // Wheel revolutions
          float distanceTraveled = wheelRev * (WHEEL_DIAMETER * M_PI) * 1000;  // Distance in mm

          // Initialize path on first entry
          if (!avoidPathActive) {
              setServoAngle(SERVO_AVOID_LEFT);  // Steer max left 45 degrees
              resetEncoder();                   // Reset encoder which is distance that motor has gone
    ...
    case MERGE_SEARCH:
      {
        ...
        if (detectJunction() || computeError() != 0) {
          state = LINE_FOLLOW;
          // first_run_after_init = false;  // Don't use soft start when resuming from merge (Commented out)
    ...
    case TURN_LEFT_PREPARE:
      ...
      if (millis() - turn_left_prepare_start >= 800) {
        ...
        state = LINE_FOLLOW;
        // first_run_after_init = false;  // Don't use soft start when resuming from turn (Commented out)
      }
      break;
